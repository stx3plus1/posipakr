...
if (opt.file_amt != count) { \
	puts("[x] incorrect number of file args >:/"); \
	print_help(); \
	destroy_quit(1); \
} \
else if (opt.file_amt >= 1 && !f1 || opt.file_amt >= 2 && !f2 || opt.file_amt >= 3 && !f3) { \
...
int main(int ac, char* av[]) {
	...
	for (int i = 1; i < ac; i++) {
		if (av[i][0] == '-') {
			for (char* p = (av[i] + 1); *p; p++) {
				switch (*p) {
				case 'c': opt.create  = 1; break;
				case 'l': opt.list    = 1; break;
				case 'e': opt.extract = 1; break;
				case 'r': opt.delete  = 1; break;
				case 'a': opt.add     = 1; break;
				case 'd': opt.dump    = 1; break;

				default: printf("[i] unknown option: %c\n", *p); break;
				}
			}
		} else {
			if      (!f1 && !f2 && !f3) f1 = av[i], opt.file_amt = 1;
			else if ( f1 && !f2 && !f3) f2 = av[i], opt.file_amt = 2;
			else if ( f1 &&  f2 && !f3) f3 = av[i], opt.file_amt = 3;
			else {
				printf("[x] additional extra argument at position %d :/", i);
				return 1;
			}
		}
	}
	...
	if (opt.add) {
		...
		pak_add_file(pak, f3, file_data, file_len);
		free(file_data);
		printf("[i] added %s as %s to pak\n", f2, f3);
	}

	if (opt.delete) {
		int ret = pak_delete_file(pak, f2);
		if (ret) {
	...
	if (opt.dump) {
		...
		dump_file = fopen(f2, "wb");
		if (!dump_file) {
			puts("[x] failed to open output file");
		}

		file_data = pak_load_file(pak, f3, &file_len);
		if (!file_data) {
	...
	if (opt.create || opt.add || opt.delete) write_posipak(pak, f1);
